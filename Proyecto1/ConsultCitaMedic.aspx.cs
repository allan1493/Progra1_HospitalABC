...
namespace Proyecto1
{
    public partial class ConsultCitaMedic : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                foreach (GridViewRow row in GVConsultaMedic.Rows)
                {
                    row.Attributes["onclick"] = Page.ClientScript.GetPostBackClientHyperlink(GVConsultaMedic, "Select$" + row.RowIndex);
                }
        ...
        private void CargaTablaCitas()
        {
            try
            {
                ...
                GVConsultaMedic.DataSource = dataTable;
                GVConsultaMedic.DataBind();
        ...
        protected void gridViewConsulta_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                ...
                GridViewRow selectedRow = GVConsultaMedic.Rows[index];
        ...
        {
            Response.Redirect("/Medic.aspx");// se manda al form de catalogos
        }
    }
