...
namespace Proyecto1
{
    public partial class Paciente_Admi : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                foreach (GridViewRow row in GVPct.Rows)
                {
                    row.Attributes["onclick"] = Page.ClientScript.GetPostBackClientHyperlink(GVPct, "Select$" + row.RowIndex);
                }
        ...
        protected void btnActualizar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //making query      
                string query = "UPDATE [Pacientes$] SET [Nombre] = '" + txt_Nombre.Text + "', [Apellido1] = '" + txt_apellido1.Text + "', [Apellido2] = '" + txt_apellido2.Text + "', [TipoID] = '" + DDLId.SelectedItem.Text + "', [Identificacion] ='" + txt_identi.Text + "', [Genero] = '" + DDLGenero.SelectedItem.Text + "', [EstadoCivil] = '" + DDLCivil.SelectedItem.Text + "', [Nacimiento] = '" + Calendar1.SelectedDate.ToShortDateString() + "', [Residencia] = '" + DDLProvincia.SelectedItem.Text + "', [Telefono] = '" + txt_TN.Text + "', [Correo] = '" + txt_CE.Text + "' WHERE [identificacion] = '" + txtGlobal.Text + "'";
                //Providing connection      
                OleDbConnection conn = new OleDbConnection(ConStr);
                //checking that connection state is closed or not if closed the       
                //open the connection      
        ...
        protected void gridViewPaciente_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                int index = Convert.ToInt32(e.CommandArgument);
                GridViewRow selectedRow = GVPct.Rows[index];
                if (e.CommandName == "Select")
                {
                    
                        
                        
                        txt_Nombre.Text = selectedRow.Cells[0].Text;
                        txt_apellido1.Text = selectedRow.Cells[1].Text;
                        txt_apellido2.Text = selectedRow.Cells[2].Text;
                        DDLId.DataTextField = selectedRow.Cells[4].Text;
                        
                        txt_identi.Text = selectedRow.Cells[3].Text;
                        txtGlobal.Text = selectedRow.Cells[3].Text.ToString();
                        DDLGenero.DataTextField = selectedRow.Cells[5].Text;
                        DDLCivil.DataTextField = selectedRow.Cells[6].Text;
                        //Calendar1.VisibleDate = DateTime.Parse(selectedRow.Cells[7].Text);
                        DDLProvincia.DataTextField = selectedRow.Cells[8].Text;
                        txt_TN.Text = selectedRow.Cells[9].Text;
                        txt_CE.Text = selectedRow.Cells[10].Text;
        ...
        protected void btnEliminar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                ConStr = "Provider=Microsoft.ACE.OLEDB.12.0;Data Source=" + path + ";Extended Properties=\"Excel 12.0;HDR=Yes;\"";
                //making query      
                string query = "UPDATE [Pacientes$] SET [Nombre] = '', [Apellido1] = '', [Apellido2] = '', [TipoID] = '', [Identificacion] ='', [Genero] = '', [EstadoCivil] = '', [Nacimiento] = '', [Residencia] = '', [Telefono] = '', [Correo] = '' WHERE [identificacion] = '" + txtGlobal.Text + "'";
                //Providing connection      
                OleDbConnection conn = new OleDbConnection(ConStr);
                //checking that connection state is closed or not if closed the       
                //open the connection      
